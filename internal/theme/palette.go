...
// Palette is the Omarchy colors.toml set used for chrome and the chessboard.
type Palette struct {
	Name string
...
func finish(p Palette) Palette {
	if p.Foreground.A == 0 {
		p.Foreground = mustHex("#efebdc")
	}
	if p.Background.A == 0 {
		p.Background = mustHex("#1B1B1B")
	}
	if p.Accent.A == 0 {
...
// GoldRush is included so a live Omarchy Gold Rush theme still parses fully.
